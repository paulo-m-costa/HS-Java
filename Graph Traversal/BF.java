//Paulo Costa
//1/15/2012
//Class to handle inputted graphs and perform Breadth-First searching to traverse all relevant nodes.

import java.io.*;
import java.util.*;

class BF implements Iterable{
   public boolean[] marker; //keeps track of visited nodes.
   Queue NextPlace;
   int[][] AdjMat2;	
   public BF(int[][] AdjMat, int start){ //constructor for handling input.
   ...
   public Iterator iterator(){ //constructor for Breadth-First exploration.
      return new BFE();
   }
   public class BFE implements Iterator{ //Breadth-First search algorithm.
      public Object next(){ //determines the next node to visit.
         int next1 = (Integer)NextPlace.peek();
         for(int b = 0; b < AdjMat2[next1].length; b++){
   ...
   public static void main(String[] args){
      ...
      BF BFE1 = new BF(AdjMat, st);
      Iterator Fpath = BFE1.iterator();
